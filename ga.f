...
c
     ...
     +                  nowrite,nichflg,microga,kountmx,scale
c
      kountmx=5
      irestrt=0
      itourny=0
...
c  Check for array sizing errors.
      ...
      open(99, file='pm3.prm')
      read(99,*)
      do i=1, 2
        read(99,*) param
        parmin(i) = scale*parmin(i) + param
        parmax(i) = scale*parmax(i) + param
      enddo 
      read(99, *) 
      do i = 3, nparam
         read(99,*) param
c         parmin(i) = 0.02*parmin(i) + param
c         parmax(i) = 0.02*parmax(i) + param
         parmin(i) = scale*parmin(i) + param
         parmax(i) = scale*parmax(i) + param
      enddo
      close(99)
...
c  to the summation for later averaging.
         call user_func(j,funcval, newParents)
         fitness(j)=funcval
         write(6, 1076) j, fitness(j), 100.d0/fitness(j)
...
c  Write output information
     ...
     +                  (parent(k,1),k=1,nparam),fitness(1)
      ...
      write(6,1100) fbar
      write(24,1100) fbar
      write(6,1200) best, 100.d0/best
      write(24,1200) best, 100.d0/best
c
c 1075 format(i3,1x,70i1,14(1x,f8.4),1x,f15.6)
 1076 format(i3,1x,f15.6, f9.2)
 1075 format(i3,1x,100i1,20(1x,f8.4),1x,f15.6)
 1100 format(1x,'Average Function Value of Generation=',f15.6)
 1200 format(1x,'Maximum Function Value              =',f15.6,f9.2/)
 1275 format(/' Average Values:',18x,20(1x,f7.4),1x,f15.6/)
      return
      end
c
c#######################################################################
...
c#######################################################################
c
      subroutine user_func(j,funcval)
c
      implicit real*8 (a-h,o-z)
      save
c
      ...
      common / ga2   / nparam,nchrome
      common / ga3   / parent,iparent
      parameter(ndata=8, ncluster=200)
      real*8 pm3_data(ndata,ncluster)
